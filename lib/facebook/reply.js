...
module.exports = function fbReply(recipient, message, fbAccessToken) {
  ...
  console.log('send single');
      ...
      if (message.hasOwnProperty('claudiaTokenOveride')) {
        messageBody.notification_type = message.notification_type;
        console.log("overriding fb token");
        fbAccessToken = message.claudiaTokenOveride;
        console.log("deleting token override from message");
        delete message.claudiaTokenOveride;
      }
      ...
      console.log('promise post; options %j', options);
      console.log("final token: " + fbAccessToken);
